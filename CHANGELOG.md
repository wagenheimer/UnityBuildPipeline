...
### Changed
- chore: Add Dashboard... priority = 0 menu item

## [1.28.0] - 2026-09-25

### Added
- ship debug PanelSettings+theme and add zoom/maximize to BuildDebugOverlay
...
### Removed
- **Redundant `CanChangeLanguage` Field**: Removed obsolete `CanChangeLanguage` boolean from `GameConfig.cs` and `GameConfigCustomEditor`. In-game language selection is already automatically enabled whenever `GameLanguage == AutoDetect`, and locked/hidden whenever a fixed language is configured.
...
### Fixed
- embed BuildPipelineConfigView in BuildPipelineWindow to resolve immutable package cache meta issue (v1.19.0)
...
### Fixed
- modernize tab navigation bar, add Discard Changes action, and dedicated Project Config view (v1.16.0)
...
### Fixed
- add missing .meta files for BuildDebugOverlay
